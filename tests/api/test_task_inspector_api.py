...
def test_api_task_inspector_returns_aggregates(api_client) -> None:
    ...
    assert payload["report_summary"]["available"] is True
    assert payload["evidence_summary"]["total_count"] >= 1
    assert payload["llm_summary"]["call_count"] == 0
    assert payload["llm_summary"]["real_llm_used"] is False
    ...
    presentations = {
        item["artifact_name"]: item for item in payload["artifact_presentations"]
    }
    assert presentations["final_report.md"]["artifact_type"] == "report"
    assert presentations["trace.jsonl"]["developer_only"] is True
