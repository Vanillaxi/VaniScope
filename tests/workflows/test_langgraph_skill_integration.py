...
def test_langgraph_prompt_includes_github_issue_skill_instruction(
    tmp_path: Path,
) -> None:
    task = build_task_spec(
        ...
        query="Analyze this GitHub issue contribution.",
        task_id="github_skill_integration",
    )
    handler = WebAgentExecutionHandler(
        output_root=tmp_path,
        workspace=Path("tests/fixtures/workspace"),
    ...
    assert "Treat the opened issue or PR page as the source of truth" in prompt
    assert "Never submit comments" in prompt
    assert workflow_state["skill_id"] == "github_issue_research"
